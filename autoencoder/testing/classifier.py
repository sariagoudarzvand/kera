...
from keras.layers import Dense, Dropout, Embedding, GlobalMaxPool1D
...
def softmax_network(input_size, n_class):
    ...
    # model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    # model = Sequential()
    # model.add(Dense(1024, activation='relu',kernel_initializer='glorot_normal', input_dim=input_size))
...
def multiclass_classifier(X_train, Y_train, X_val, Y_val, X_test, Y_test, nb_epoch=200, batch_size=10, seed=7):
    ...
    clf.fit(X_train, Y_train,
                        ...
                        callbacks=[
                                    ...
                                    EarlyStopping(monitor='val_loss', min_delta=1e-5, patience=25, verbose=0, mode='auto'),
                        ]
                        )
...
def multilabel_classifier(X_train, Y_train, X_val, Y_val, X_test, Y_test, nb_epoch=200, batch_size=10, seed=7):
    ...
    clf.fit(X_train, Y_train,
                        ...
                        callbacks=[
                                    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=3, min_lr=0.01),
                                    EarlyStopping(monitor='val_loss', min_delta=1e-5, patience=20, verbose=0, mode='auto'),
                        ]
                        )
